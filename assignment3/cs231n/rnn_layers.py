...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    for i in range(x.shape[1]):
        h0, temp = rnn_step_forward(x[:, i, :], h0, Wx, Wh, b)
        cache.append(temp)
        h[:, i, :] = h0
    ##############################################################################
    #                               END OF YOUR CODE                             #
...
def rnn_backward(dh, cache):
    ...
    N, T, H = dh.shape
    D = cache[0][2].shape[0]
    dx, dh0, dWx, dWh, db = np.zeros((N, T, D)), 1, np.zeros((D, H)), np.zeros((H, H)), np.zeros((H))
    ##############################################################################
    ...
    for i in range(T - 1, -1, -1):
        temp = rnn_step_backward(dh[:, i, :] + tdprev_h, cache[i])
        # print(temp[1])
        # print(dh[:,i,:])
        dx[:, i, :] = temp[0]
...
def lstm_step_forward(x, prev_h, prev_c, Wx, Wh, b):
    ...
    cache = x, Wx, Wh, temp, next_c, prev_c, prev_h
    ##############################################################################
    #                               END OF YOUR CODE                             #
    ##############################################################################
...
def lstm_step_backward(dnext_h, dnext_c, cache):
    ...
    #############################################################################
    x, Wx, Wh, temp, next_c, prev_c, prev_h = cache
    H = dnext_h.shape[1]
    i = sigmoid(temp[:, :H])
    f = sigmoid(temp[:, H:2 * H])
    o = sigmoid(temp[:, 2 * H:3 * H])
...
def lstm_forward(x, h0, Wx, Wh, b):
    ...
    #############################################################################
    for i in range(x.shape[1]):
        h0, c0, temp = lstm_step_forward(x[:, i, :], h0, c0, Wx, Wh, b)
        cache.append(temp)
        h[:, i, :] = h0
    ##############################################################################
    #                               END OF YOUR CODE                             #
...
def lstm_backward(dh, cache):
    ...
    dh0 = np.zeros((N, H))
    for i in range(T - 1, -1, -1):
        temp = lstm_step_backward(dh[:, i, :] + dh0, dc0, cache[i])
        dx[:, i, :], dh0, dc0, dWx0, dWh0, db0 = temp
        dWx+=dWx0
        dWh+=dWh0
        db+=db0
    ##############################################################################
